...
| Default TUI, Desktop, dashboard, messaging adapters | Attached clients | Resolve the same gateway conversation owner and use presentation adapters around its event stream. | Supported where the adapter contract is enabled. Multiple clients may observe and interact with one runtime. | Canonical runtime owner only. |
