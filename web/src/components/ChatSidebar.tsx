 ...
 *      backoff; auth rejections are terminal. This PTY side feed has no
 *      durable cursor or missed-event replay, so reconnect resumes with new
 *      events only. See `lib/events-reconnect`.
 *
...
export function ChatSidebar({
  ...
  const gw = useMemo(() => {
    ...
    return new GatewayClient({
      onDurableResyncRequired: () => setVersion(current => current + 1)
    })
  }, [version])
